...
confHook
    :: (GenericPackageDescription.GenericPackageDescription, HookedBuildInfo.HookedBuildInfo)
    -> Cabal.ConfigFlags
...
discoverPackageModules gpd = do
    let
        ...
        hsSourceDirs = Set.toList . Set.unions $ fmap getHsSourceDirs buildInfos
        toMap directory = Map.singleton directory
...
discoverPackageModulesWith gpd directoryContents = gpd
    ...
    , GenericPackageDescription.condSubLibraries =
        overSnd (CondTree.mapTreeData $ discoverLibraryModules directoryContents)
        <$> GenericPackageDescription.condSubLibraries gpd
    , GenericPackageDescription.condForeignLibs =
        overSnd (CondTree.mapTreeData $ discoverForeignLibModules directoryContents)
        <$> GenericPackageDescription.condForeignLibs gpd
    , GenericPackageDescription.condExecutables =
        overSnd (CondTree.mapTreeData $ discoverExecutableModules directoryContents)
        <$> GenericPackageDescription.condExecutables gpd
    , GenericPackageDescription.condTestSuites =
        overSnd (CondTree.mapTreeData $ discoverTestSuiteModules directoryContents)
        <$> GenericPackageDescription.condTestSuites gpd
    , GenericPackageDescription.condBenchmarks =
        overSnd (CondTree.mapTreeData $ discoverBenchmarkModules directoryContents)
        <$> GenericPackageDescription.condBenchmarks gpd
    }
...
discoverBenchmarkModules :: Map.Map FilePath [FilePath] -> Benchmark.Benchmark -> Benchmark.Benchmark
...
discoverTestSuiteModules :: Map.Map FilePath [FilePath] -> TestSuite.TestSuite -> TestSuite.TestSuite
...
discoverExecutableModules :: Map.Map FilePath [FilePath] -> Executable.Executable -> Executable.Executable
...
discoverForeignLibModules :: Map.Map FilePath [FilePath] -> ForeignLib.ForeignLib -> ForeignLib.ForeignLib
...
discoverLibraryModules :: Map.Map FilePath [FilePath] -> Library.Library -> Library.Library
discoverLibraryModules directoryContents =
    discoverOtherLibraryModules directoryContents . discoverExposedLibraryModules directoryContents

discoverExposedLibraryModules :: Map.Map FilePath [FilePath] -> Library.Library -> Library.Library
    ...
    case maybeRemove velmaDiscover $ Library.exposedModules library of
        ...
        Just exposedModules ->
            let
                ...
                entries = concat . Map.elems $ Map.restrictKeys directoryContents directories
                otherModules = Set.fromList . BuildInfo.otherModules $ Library.libBuildInfo library
                discovered = filter (`Set.notMember` otherModules)
...
discoverOtherLibraryModules :: Map.Map FilePath [FilePath] -> Library.Library -> Library.Library
discoverOtherLibraryModules directoryContents library =
    let oldBuildInfo = Library.libBuildInfo library
    in case maybeRemove velmaDiscover $ BuildInfo.otherModules oldBuildInfo of
        Nothing -> library
        Just otherModules ->
            let
                directories = getHsSourceDirs oldBuildInfo
                entries = concat . Map.elems $ Map.restrictKeys directoryContents directories
                exposedModules = Set.fromList $ Library.exposedModules library
                discovered = filter (`Set.notMember` exposedModules)
                    $ Maybe.mapMaybe filePathToModuleName entries
                allModules = ListUtils.nubOrd $ otherModules <> discovered
                newBuildInfo = oldBuildInfo { BuildInfo.otherModules = allModules }
            in library { Library.libBuildInfo = newBuildInfo }

discoverOtherModules :: Map.Map FilePath [FilePath] -> BuildInfo.BuildInfo -> BuildInfo.BuildInfo
...
foldTraverse :: (Applicative m, Monoid b, Traversable t) => (a -> m b) -> t a -> m b
...
listDirectoryHelper entry = do
    ...
    if isDirectory
        then listDirectoryRecursively entry
        else pure [entry]
